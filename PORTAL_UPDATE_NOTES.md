...
- Исправлена проверка образов после `docker load` в Docker с containerd image store: для локальных `kvn-*` образов verifier экспортирует загруженный tag через `docker image save` и строго сверяет исходный config digest. Это устраняет ложную ошибку `загруженный образ не совпадает: kvn-portal:local` без ослабления проверки.
